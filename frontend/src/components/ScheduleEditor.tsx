 ...
 *
 * Công tắc để ở BẬT (mặc định) là AI chạy liên tục 24/7. Tắt công tắc xuống thì mới
 * hiện khung ngày/giờ để chọn — lúc đó AI chỉ chạy trong các khung giờ đã khai báo;
 * mỗi khung gồm những thứ trong tuần và một khoảng giờ. Nhiều khung ghép được với
 * nhau, ví dụ giờ hành chính các ngày trong tuần cộng thêm buổi sáng thứ Bảy.
 *
...
/** Mặc định chạy 24/7: `enabled=false` khiến công tắc hiện ở vị trí BẬT. */
    ...
    <div className="rounded-xl border border-slate-200 bg-white p-3">
      <div className="flex items-center justify-between">
        ...
        <button
          ...
          aria-label="Chạy theo khung giờ"
        ...
        >
          {/* Núm trượt: track rộng 44px, núm 20px, chừa 2px mỗi bên nên quãng dịch
              ...
              Công tắc hiển thị đảo ngược với `enabled`: BẬT = chạy 24/7. */}
          <span
      ...
      <div className="mt-3 space-y-2">
        {value.slots.map((slot, index) => (
          <div key={index} className="rounded-lg bg-slate-50 p-3">
            ...
            {slot.from && slot.to && slot.from.length >= 4 && slot.to.length >= 4
