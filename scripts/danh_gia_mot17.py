...
# Cửa sổ ghép cặp. 45 khung hình là 1,5 giây ở 30 fps — đủ rộng cho độ trễ xác nhận
# của bộ đếm, đủ hẹp để không ghép nhầm hai người khác nhau đi qua liên tiếp.
LECH_KHUNG = 45
LECH_DIEM_ANH = 200
...
def _duong_dan(chuoi: str) -> tuple[Path, Path]:
    so = chuoi.split("-")[1]
    return GOC / "videos" / f"mot17-{so}.mp4", \
        GOC / "datasets" / "MOT17" / chuoi / "gt" / "gt.txt"
...
def _phat_hien(video: Path, conf: float | None = None):
    ...
    det = D.Detector(["person"], conf=conf, fps=30)
    khung = []
    while True:
        ok, f = cap.read()
        if not ok:
...
def _do_mot_chuoi(chuoi: str, vi_tri, ngang: bool, khung, W, H, vet) -> list[tuple]:
    ra = []
    for x in vi_tri:
        if ngang:
